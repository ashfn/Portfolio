...
<head>
    <title>Code coverage report for src/pages/list.tsx</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">src/pages</a> list.tsx</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { GraphQLError } from 'graphql'
...
&nbsp;
export default function <span class="fstat-no" title="function not covered" >List(</span>{ collection }: ListProps) {
  const {
    repoOwner,
    repoSlug,
    repoBranch,
  ...
  const { data, error, loading } = <span class="cstat-no" title="statement not covered" >useDocumentsQuery({</span>
    variables: {
    ...
    onError: <span class="fstat-no" title="function not covered" >({</span> graphQLErrors }) =&gt; {
<span class="cstat-no" title="statement not covered" >      if (</span>
        graphQLErrors &amp;&amp;
        (graphQLErrors?.[0] as GQLErrorExtended)?.type === 'NOT_FOUND'
      ) {
<span class="cstat-no" title="statement not covered" >        ostSignOut()</span>
...
&nbsp;
  let documents: Omit&lt;Document, 'content'&gt;[] = <span class="cstat-no" title="statement not covered" >[]</span>
&nbsp;
  const entries =
<span class="cstat-no" title="statement not covered" >    data?.repository?.object?.__typename === 'Tree' &amp;&amp;</span>
    data?.repository?.object?.entries
&nbsp;
<span class="cstat-no" title="statement not covered" >  if (entries) {</span>
<span class="cstat-no" title="statement not covered" >    entries.forEach(<span class="fstat-no" title="function not covered" >(d</span>ocument) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      if (document.name.slice(-3) === '.md') {</span>
        const {
          data: { title, publishedAt, status, author }
        } = <span class="cstat-no" title="statement not covered" >matter(</span>
          document?.object?.__typename === 'Blob' &amp;&amp; document?.object?.text
            ? document?.object?.text
            : ''
        )
<span class="cstat-no" title="statement not covered" >        documents.push({</span>
          title,
          status,
...
&nbsp;
<span class="cstat-no" title="statement not covered" >    documents.sort(<span class="fstat-no" title="function not covered" >(a</span>, b) =&gt; <span class="cstat-no" title="statement not covered" >Number(b.publishedAt) - Number(a.publishedAt))</span></span>
  }
&nbsp;
